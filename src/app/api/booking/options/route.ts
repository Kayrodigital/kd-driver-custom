...
export async function POST(request: Request) {
  try {
    ...
    const options = Object.keys(pricingConfig.categories).map((category) => ({ category, pricing: calculatePrice({ category, distanceMeters: route.distanceMeters, isAirportTrip: input.isAirportTrip }, pricingConfig) }));
